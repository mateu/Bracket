...
sub login : Global {
	...
	if ( !$is_authenticated ) {
		...
		$c->response->body(
			"Could not <big><a href='$login_URI'>login</a></big>");
		$c->detach();
	}
...
sub reset_password : Global {
	...
	my $token_row_object =
	  $c->model('DBIC::Token')
	  ->search(
		{ player => $user_id, token => $token, type => 'reset_password' } )
	  ->first;
	if ( !$token_row_object ) {
...
sub email_link : Private {
	...
	my $link =
	  $c->request->base . 'reset_password?reset_password_token=' . $token;
...
END_MESSAGE
	...
	Email::Stuff->to($to_email)->from($admin_email)->subject($subject)
	  ->text_body($message)->send;
}
